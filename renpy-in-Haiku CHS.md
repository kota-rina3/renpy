...
接下来安装编译renpy所需依赖：
`pkgman install pkgconf freetype_devel ffmpeg6_avdevice ffmpeg6_devel fribidi_devel assimp_devel libsdl3_devel sdl3_image_devel sdl3_mixer_devel sdl3_sound_devel sdl3_ttf_devel`

安装所需python模块：
`pip3.12 install assimp cython future legacy-cgi pefile requests six typing`
...
做完这些，静静等待renpy编译完成
最后输入`python3.12 -X utf8 renpy.py`并等待窗口出现
