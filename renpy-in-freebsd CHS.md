...
从系统源里安装编译所需依赖：`pkg install bash assimp sdl3 sdl3_image sdl3_mixer sdl3_sound sdl3_ttf pkgconf freetype2 harfbuzz fribidi`
...
终端里执行uv同步和编译脚本：`uv sync && ./run.sh --build`

最后，renpy会自行启动。
要想再次启动renpy，执行`python3.12 -X utf8 renpy.py`
...
从pip里安装所需模块：`~/.local/bin/pip3.12 install setuptools cython future six typing pefile requests assimp legacy-cgi`
> 速度慢？可挂镜像源加速下载：-i https://mirrors.aliyun.com/pypi/simple/
将game文件夹放进renpy-box并执行python3.12 -X utf8 renpy.py，等一会就能玩了
